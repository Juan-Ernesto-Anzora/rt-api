...
- `tests/test_lookups.py` for web create-request lookup endpoints
- optional updates to `tests/test_request_detail.py` if shared serializers change

### Milestone 3a: Web create-request lookup endpoints

The web Create Request page needs tenant-scoped lookup data so users select names instead of typing GUIDs. Add these generated-schema endpoints:

- `GET /api/flows/`
- `GET /api/flows/{flow_id}/statuses/`
- `GET /api/users/?search=`

Behavior:

- All endpoints require JWT through DRF permissions and `X-Tenant` through `TenantMiddleware`.
- `GET /api/flows/` returns tenant-scoped `flow_id` and `name`.
- `GET /api/flows/{flow_id}/statuses/` first verifies the flow belongs to the active tenant, then returns statuses with `status_id`, `flow_id`, `name`, `category`, and `is_terminal`. The web can identify default opening statuses with `category = "open"`.
- `GET /api/users/?search=` returns users who belong to the active tenant through `Membership`, with `user_id`, `display_name`, and `email`; optional `search` filters display name or email.
- Cross-tenant flow status lookup returns a clean `404`.
- OpenAPI entries are generated by drf-spectacular from registered routers and explicit lookup serializers.
...
- [x] Milestone 6 completed.
  - [x] Postman/MailHog regression fixed: assignment and closed notifications are domain-triggered and email bodies include request links.
...
- The notification baseline is small enough for a synchronous service module. There is still no Celery worker wiring in this repo, so the baseline must avoid letting SMTP failures affect request writes.
- MailHog testing showed created/comment notifications worked, but assignment needed reliable assignee resolution and closed needed to key off status category changes instead of the `/close/` action type.
- The web request detail URL is not generated by the API yet, so notification bodies use a configurable placeholder based on `WEB_BASE_URL`.
...
- Add Mailhog-oriented SMTP settings with `localhost:1025` defaults and `DEFAULT_FROM_EMAIL=rt-api@localhost`.
- Send baseline notification emails synchronously for request created, request assigned, comment added, and request closed; the service deduplicates recipients and catches email exceptions.
- Notify requester and assignee as request stakeholders. Send assignment email only to the assignee when one exists.
- Treat grouped attachment finalize as a comment-added event because the upload flow creates one grouped comment bubble.
- Add `WEB_BASE_URL` with local default `http://127.0.0.1:5173` and include `Link: {WEB_BASE_URL}/requests/{request_id}` in all notification bodies.
- Trigger assignment notifications when a request is created with an assignee or when `assignee_id` changes to a non-null different user through request updates.
- Trigger closed notifications whenever status category changes from non-closed to `closed`, including generic `/transition/` calls.
...
- Notification baseline now sends Mailhog-visible email events for request create, assignment, comment add, grouped upload comment add, and close.
